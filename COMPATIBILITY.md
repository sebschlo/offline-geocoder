...
|---|---|---|---|---|
| 5 | Compact v2 + centroid validity | Pending | `compact_places` (+ nullable `centroid_inside`) + `compact_geohash_lookup` | A place whose stored coordinates fall outside its own geometry (a bounding-box fallback for a concave, holed, or multipart shape) must not claim the geohash cell holding that point. The builder decides this once, at normalization; the flag persists the decision so an `--append` batch cannot recreate the claim from coordinates alone. `NULL` means "not recorded", which keeps the pre-upgrade behaviour of trusting the coordinates. Readers ignore the column. |
