...
```javascript
var geocoder = require('offline-geocoder')({ database: 'data/geocoder.sqlite' })
```
...
Returns `{}` when no nearby city is found (keeps backwards compatibility).
...
```javascript
var createGeocoder = require('offline-geocoder/expo')

var db = await SQLite.openDatabaseAsync('geocoder.sqlite')
var geocoder = createGeocoder({ db: db })
